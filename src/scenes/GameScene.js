...
class GameScene extends Phaser.Scene {
    ...
    preload() {
        this.load.image('bg', 'src/image/bgthai.png')
        // this.load.spritesheet('player','src/image/Ermine.png',
        // { frameWidth: 528 , frameHeight: 458});
        // this.load.image('shark','src/image/Ermine.png',
        // // {frameWidth: 580 , frameHeight: 284}
        // );
        this.load.image('sky','src/image/Skybg.png');
        this.load.image('trident','src/image/TuuSub.png');
        this.load.spritesheet('bomb', 'src/image/money.png',{frameWidth: 250, frameHeight: 250});
        this.load.audio('bulletSound', 'src/sound/Trident.mp3');
        this.load.audio('bgsound','src/sound/game.mp3');
    ...
    create() {
        ...
        skybox = this.physics.add.image(750, 0, 'sky')
            ...
            .setVisible()
            .setImmovable();
        p1 = this.physics.add.sprite(200, 420, 'player')
            .setScale(0.25)
            .setSize(400, 229)
            .setOffset(50, 100);
    ...
    update(delta, time) {
        ...
        //Speed up when time increse
            sharkSpeed = Phaser.Math.GetSpeed(-200,1)*delta;
